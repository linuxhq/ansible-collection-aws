...
* Support check mode
  ...
  * Skip follow-up aws reads or validation tasks in check mode when the data
    will not exist yet

* Support `state: present` and `state: absent` for resource-management roles
  when the underlying module supports both states

* Keep roles idempotent and avoid unnecessary aws api calls
...
* Tag management
  * Preserve caller-provided tags and add the resource `Name` tag where nearby
    roles do so
  * Default `purge_tags` consistently with nearby roles, usually `true` for
    primary resources and `false` for helper tag updates
...
* Do not introduce unprefixed facts except for the established role outputs that
  begin with a single underscore, such as `_ec2_vpc_net_info_list`
...
* Use the collection fqcn in role examples, such as `linuxhq.aws.<role_name>`
...
* Keep task names consistent across roles; majority convention wins
  * `Ensure information about aws <resources> is gathered`
  * `Ensure list of aws <resources> is generated`
  * `Ensure dict of aws <resources> is generated`
  * `Ensure aws <resources> are batched`
  * `Ensure aws <resources> are managed`
  * `Ensure managed aws <resource> jobs are complete`
...
* Set `validate_certs: true` on aws module calls unless an existing role
  deliberately exposes a variable for it
...
* Prefer explicit loops and facts over dense Jinja expressions when the logic
  filters or reshapes aws resources in multiple steps
